...
CMD:factionhelp(playerid)
{
	if(pData[playerid][pFaction] == 1)
	{
		...
		strcat(str, ""BLUE_E"SAPD: /takecrack /takepot /createlicense /spike /destroyspike /impound /pdveh /destroyallspike\n");
	...
	else if(pData[playerid][pFaction] == 3)
	{
		...
		strcat(str, ""WHITE_E"SAMD: /samdonline /loadinjured /dropinjured /ems /findems /rescue /salve /berobat\n");
		strcat(str, ""WHITE_E"NOTE: Lama waktu duty anda akan menjadi gaji anda pada saat paycheck!\n");
...
CMD:r(playerid, params[])
{
	...
	if(pData[playerid][pTogRadio] == 1)
		return Error(playerid, "Your radio is turned off!");
		
  	if(pData[playerid][pFaction] == 1) {
   		SendFactionMessage(1, COLOR_RADIO, "%s %s: %s", GetFactionRank(playerid), pData[playerid][pName], text);
		format(mstr, sizeof(mstr), "[<RADIO>]\n* %s *", text);
		SetPlayerChatBubble(playerid, mstr, COLOR_PINK, 10.0, 3000);
    }
    else if(pData[playerid][pFaction] == 2) {
    	SendFactionMessage(2, COLOR_RADIO, "%s %s: %s", GetFactionRank(playerid), pData[playerid][pName], text);
		format(mstr, sizeof(mstr), "[<RADIO>]\n* %s *", text);
		SetPlayerChatBubble(playerid, mstr, COLOR_PINK, 10.0, 3000);
	}
   	else if(pData[playerid][pFaction] == 3) {
    	SendFactionMessage(3, COLOR_RADIO, "%s %s: %s", GetFactionRank(playerid), pData[playerid][pName], text);
		format(mstr, sizeof(mstr), "[<RADIO>]\n* %s *", text);
		SetPlayerChatBubble(playerid, mstr, COLOR_PINK, 10.0, 3000);
	}
   	else if(pData[playerid][pFaction] == 4) {
     	SendFactionMessage(4, COLOR_RADIO, "%s %s: %s", GetFactionRank(playerid), pData[playerid][pName], text);
		format(mstr, sizeof(mstr), "[<RADIO>]\n* %s *", text);
		SetPlayerChatBubble(playerid, mstr, COLOR_PINK, 10.0, 3000);
	}
    else
...
CMD:d(playerid, params[])
{
	...
	for(new fid = 1; fid < 5; fid++)
	{
		if(pData[playerid][pFaction] == 1) 
		{
			SendFactionMessage(fid, COLOR_RADIO, "SAPD Departement: %s(%d) %s: %s", GetFactionRank(playerid), pData[playerid][pFactionRank], pData[playerid][pName], text);
			format(mstr, sizeof(mstr), "[<DEPARTEMENT>]\n* %s *", text);
			SetPlayerChatBubble(playerid, mstr, COLOR_PINK, 10.0, 3000);
		}
		else if(pData[playerid][pFaction] == 2) 
		{
			SendFactionMessage(fid, COLOR_RADIO, "SAGS Departement: %s(%d) %s: %s", GetFactionRank(playerid),  pData[playerid][pFactionRank], pData[playerid][pName], text);
		}
		else if(pData[playerid][pFaction] == 3) 
		{
			SendFactionMessage(fid, COLOR_RADIO, "SAMD Departement: %s(%d) %s: %s", GetFactionRank(playerid),  pData[playerid][pFactionRank], pData[playerid][pName], text);
			format(mstr, sizeof(mstr), "[<DEPARTEMENT>]\n* %s *", text);
			SetPlayerChatBubble(playerid, mstr, COLOR_PINK, 10.0, 3000);
		}
		else if(pData[playerid][pFaction] == 4) 
		{
			SendFactionMessage(fid, COLOR_RADIO, "SANA Departement: %s(%d) %s: %s", GetFactionRank(playerid),  pData[playerid][pFactionRank], pData[playerid][pName], text);
			format(mstr, sizeof(mstr), "[<DEPARTEMENT>]\n* %s *", text);
			SetPlayerChatBubble(playerid, mstr, COLOR_PINK, 10.0, 3000);
		}
		else
...
CMD:givesim(playerid, params[])
{
	...
	SendClientMessageEx(playerid, COLOR_ARWIN, "LICENSE: "WHITE_E"Anda telah berhasil memberikan surat izin mengemudi kepada "YELLOW_E"%s", ReturnName(otherid));
	SendClientMessageEx(otherid, COLOR_ARWIN, "LICENSE: "YELLOW_E"%s "WHITE_E"telah berhasil memberikan surat izin mengemudi kepada anda", ReturnName(playerid));
	...
	if(pData[playerid][pFaction] == 1)
	{
		...
		switch(type)
		{
			case 1:
			{
				...
				format(String, sizeof(String), "You have given a Weapon license to %s.", pData[giveplayerid][pName]);
				SendClientMessageEx(playerid, COLOR_WHITE, String);
				format(String, sizeof(String), "%s has given you a Weapon license.",pData[playerid][pName]);
			...
			case 2:
			{
				...
				format(String, sizeof(String), "You have given a Trucking license to %s.",pData[giveplayerid][pName]);
				SendClientMessageEx(playerid, COLOR_WHITE, String);
				format(String, sizeof(String), "%s has given you a Trucking license.",pData[playerid][pName]);
				SendClientMessageEx(giveplayerid, COLOR_WHITE, String);
			...
			case 3:
			{
				...
				format(String, sizeof(String), "You have given a Lumberjack license to %s.",pData[giveplayerid][pName]);
				SendClientMessageEx(playerid, COLOR_WHITE, String);
				format(String, sizeof(String), "%s has given you a Lumberjack license.",pData[playerid][pName]);
...
CMD:release(playerid, params[])
{
	...
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 226.8309, 114.6640, 999.0156) || IsPlayerInRangeOfPoint(playerid, 3.0, 200.6972,168.4792,1003.0234))
	...
	SendFactionMessage( 1, COLOR_BLUE, "[PRISON]"WHITE_E"Officer %s telah membebaskan %s dari penjara.", ReturnName(playerid), ReturnName(otherid));
	return true;
}
...
CMD:arrest(playerid, params[])
{
    ...
    if(sscanf(params, "uddd", otherid, cellid, time, denda))
    ...
    SendFactionMessage( 1, COLOR_BLUE, "[PRISON]"WHITE_E" %s telah ditangkap dan dipenjarakan oleh polisi selama %d hari dengan denda "GREEN_E"%s.", ReturnName(otherid), time, FormatMoney(denda));
    return 1;
}
	...
	foreach(new ii : PVehicles)
	{
		if(vehid == pvData[ii][cVeh])
		{
			...
			{	
				...
				SendClientMessageEx(playerid, COLOR_ARWIN, "TICKET: "WHITE_E"Anda telah menilang kendaraan {00FFFF}%s(id: %d) "WHITE_E"dengan denda sejumlah "RED_E"%s", GetVehicleName(vehid), vehid, FormatMoney(ticket));
				return 1;
			}
...
CMD:checkveh(playerid, params[])
{
	...
	if((carid = Vehicle_Nearest(playerid)) != -1)
	{
		...
		if(rows) 
		{
			...
			{
				SendClientMessageEx(playerid, COLOR_ARWIN, "VEHICLE: "WHITE_E"ID: %d | Model: %s | Owner: %s | Plate: %s | Plate Time: %s", vehicleid, GetVehicleName(vehicleid), owner, pvData[carid][cPlate], ReturnTimelapse(gettime(), pvData[carid][cPlateTime]));
			}
			else
			{
...
CMD:takepot(playerid, params[])
{
	...
	if(sscanf(params, "u", otherid))
	{
	    Usage(playerid, "/pot <ID/Name> | Melenyapkan pot");
	    return true;
	}
	...
	SendClientMessageEx(playerid, COLOR_ARWIN, "TAKE: "WHITE_E"Anda telah mengambil semua pot milik %s.", ReturnName(otherid));
	SendClientMessageEx(otherid, COLOR_ARWIN, "TAKE: "WHITE_E"Officer %s telah mengambil semua poy milik anda", ReturnName(playerid));
	return 1;
}

CMD:takecrack(playerid, params[])
{
	if(pData[playerid][pFaction] != 1)
        return Error(playerid, "You must be a sapd officer.");
	if(pData[playerid][pFactionRank] < 1)
	...
	if(sscanf(params, "u", otherid))
	{
	    Usage(playerid, "/takecrack <ID/Name> | Melenyapkan crack");
	    return true;
	}
	...
	SendClientMessageEx(playerid, COLOR_ARWIN, "TAKE: "WHITE_E"Anda telah mengambil semua crack milik %s.", ReturnName(otherid));
	SendClientMessageEx(otherid, COLOR_ARWIN, "TAKE: "WHITE_E"Officer %s telah mengambil semua crack milik anda", ReturnName(playerid));
	return 1;
}
...
CMD:takedl(playerid, params[])
{
	...
	SendClientMessageEx(playerid, COLOR_ARWIN, "LICENSE: "WHITE_E"Anda telah menilang Driving License milik %s.", ReturnName(otherid));
	SendClientMessageEx(otherid, COLOR_ARWIN, "LICENSE: "WHITE_E"Officer %s telah menilang Driving License milik anda", ReturnName(playerid));
	return 1;
}
...
CMD:trace(playerid, params[])
{
	...
	if(sscanf(params, "u", id))
        return Usage(playerid, "/trace [playerid/PartOfName]");

	if(id == INVALID_PLAYER_ID)
		return Error(playerid, "Invalid playerid!");

	if(pData[playerid][pFaction] != 1)
		return Error(playerid, "You not SAPD");

	if(pData[playerid][pFactionRank] > 3) return Error(playerid, "You must faction level 3 - 6");
	...
	if(GetPlayerInterior(id) >= 1 || GetPlayerVirtualWorld(id) >= 1)
		return SendClientMessage(playerid, COLOR_GREY, "Player ini berada pada interior alternatif atau di dalam Virtual World.");
	
	if(pData[playerid][pAdminDuty] >= 1)
		return SendClientMessage(playerid, COLOR_GREY, "Anda tidak dapat melacak orang ini pada saat ini.");

	GetPlayerPos(id, FindFloats[0], FindFloats[1], FindFloats[2]);
	format(String, sizeof(String), "TRACKER: "YELLOW_E"Lokasi {00FFFF}%s "YELLOW_E"terlacak sedang berada di daerah {00FFFF}%s", ReturnName(id), GetLocation(FindFloats[0], FindFloats[1], FindFloats[2]));
...
CMD:loadinjured(playerid, params[])
{
    ...
    if(GetVehicleModel(i) == 416)
    {
        ...
        SamdInfo(otherid, "You're injured ~r~now you're on ambulance.");
        return 1;
    }
...
CMD:dropinjured(playerid, params[])
{
    ...
    if(IsPlayerInRangeOfPoint(playerid, 5.0, 1142.38, -1330.74, 13.62))
    {
        ...
        SamdInfo(playerid, "You have delivered %s to the hospital.", ReturnName(otherid));
        SamdInfo(otherid, "You have recovered at the nearest hospital by officer %s.", ReturnName(playerid));
    }
...
CMD:findems(playerid, params[])
{
	...
	SamdInfo(otherid, "Officer %s sedang menuju ke lokasi anda. harap tunggu!", ReturnName(playerid));
	return 1;
}
...
CMD:rescue(playerid, params[])
{
    ...
    SamdInfo(otherid, "Officer %s has rescue your character.", ReturnName(playerid));
	return 1;
}

CMD:berobat(playerid, params[])
{
	if(pData[playerid][pFaction] != 3)
        return Error(playerid, "You must be a samd officer.");
	...
	new otherid;
	if(sscanf(params, "u", otherid))
        return Usage(playerid, "/berobat [playerid/PartOfName]");
	...
	SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "* %s has treatment %s with medkit.", ReturnName(playerid), ReturnName(otherid));
    SamdInfo(otherid, "Officer %s has treatment your character.", ReturnName(playerid));
	return 1;
}
...
CMD:salve(playerid, params[])
{
    ...
    SamdInfo(otherid, "Officer %s has resalve your sick character.", ReturnName(playerid));
	return 1;
}
...
CMD:impound(playerid, params[])
{
	...
	new vehicleid;
	if(sscanf(params, "d", vehicleid)) return Usage(playerid, "/impound [vehid] | /pdveh - for find vehid");

	vehicleid = GetNearestVehicleToPlayer(playerid, 3.5, false);
	if(vehicleid == INVALID_VEHICLE_ID)
		return Error(playerid, "You not in near any vehicles.");
	ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
	pData[playerid][pActivity] = SetTimerEx("ImpoundVeh", 1000, true, "id", playerid, vehicleid);
	PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Impound");
...
CMD:tirelock(playerid, params[])
{
	if(pData[playerid][pFaction] != 1)
        return Error(playerid, "Anda bukan petugas kepolisian.");
	new vehicleid;
	if(sscanf(params, "d", vehicleid)) return Usage(playerid, "/tirelock [vehid] | /pdveh - for find vehid");
	...
	foreach(new ii : PVehicles)
	{
		...
		if(vehicleid == pvData[ii][cVeh])
		{
        	...
        	{
			    format(S3MP4K, sizeof(S3MP4K), "TIRELOCK: {FFFFFF}Your {FFFF00}%s {FFFFFF}has been tirelock by {00FFFF}%s.", GetVehicleName(vehicleid), ReturnName(playerid));
				SendClientMessageEx(pid, COLOR_ARWIN, S3MP4K);	
				pvData[ii][cImpound] = 2;	
				pvData[ii][cTicketTime] = 3600;
				pvData[ii][cLabelTire] = CreateDynamic3DTextLabel("Tire Lock",COLOR_BLUE,0.0, 0.0, 0.0,5.0,INVALID_PLAYER_ID,pvData[ii][cVeh],1);
			}
		}
	}
	return 1;
...
CMD:segelbiz(playerid, params[])
{
	...
	foreach(new bid : Bisnis)
	{
		if(IsPlayerInRangeOfPoint(playerid, 2.5, bData[bid][bExtposX], bData[bid][bExtposY], bData[bid][bExtposZ]))
		{
			...
			SendFactionMessage(3, COLOR_ARWIN, "SAGS: {00FFFF}%s "WHITE_E"has sealed the "YELLOW_E"ID %d "WHITE_E"business", pData[playerid][pName], bid);
		}
	}
	return 1;
}
...
CMD:unsegelbiz(playerid, params[])
{
	...
	foreach(new bid : Bisnis)
	{
		if(IsPlayerInRangeOfPoint(playerid, 2.5, bData[bid][bExtposX], bData[bid][bExtposY], bData[bid][bExtposZ]))
		{
			...
			SendFactionMessage(3, COLOR_ARWIN, "SAGS: {00FFFF}%s "WHITE_E"has unsealed the "YELLOW_E"ID %d "WHITE_E"business", pData[playerid][pName], bid);
		}
	}
	return 1;
}
...
CMD:tracephone(playerid, params[])
{
	...
	if (!IsSAPDCar(GetPlayerVehicleID(playerid)))
	    return Error(playerid, "You must be inside a police vehicle.");
...
CMD:traceplate(playerid, params[])
{
	...
	if(sscanf(params, "s[32]", no))
        return Usage(playerid, "/traceplate [plate]");
