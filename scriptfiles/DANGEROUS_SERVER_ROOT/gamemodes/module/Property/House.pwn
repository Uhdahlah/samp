...
House_Refresh(houseid)
{
    if(houseid != -1)
    {
		...
		{
			format(string, sizeof(string), "{00FFFF}[id:%d]\n{00FF00}Owner: {FFFFFF}%s{00FF00}\nAddres: {FFFFFF}%s", houseid, hData[houseid][hOwner], GetLocation(hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ]));
...
CMD:createhouse(playerid, params[])
{
	...
	new type, query[512];
	if(sscanf(params, "d", type)) return Usage(playerid, "/createhouse [type, 1.small 2.medium 3.Big]");
