...
//-----[ Modular Server ]-----
//#include "./module/Server/Audio.pwn"
...
#include "./module/Server/anticheat.pwn"
//-----[ Modular Property]-----
...
#include "./module/Jobs/PizzaJobs.pwn"
#include "./module/Jobs/DepositorJobs.pwn"
...
#include "./module/Jobs/TrashJobs.pwn"
//-----[ Modular Command]-----
...
public OnGameModeInit()
{
	...
	//g_SQL = mysql_connect("localhost", "root", "", "jgrp", option_id);
	...
	mysql_tquery(g_SQL, "SELECT * FROM `actor`", "ActorLoad");
	...
	mysql_tquery(g_SQL, "SELECT * FROM `flat`", "Flat_Load");
  	...
  	mysql_tquery(g_SQL, "SELECT * FROM `gymmachine`", "LoadGYMObject");
  	mysql_tquery(g_SQL, "SELECT * FROM `gympoint`", "LoadGym");
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	if(pData[playerid][IsLoggedIn] == true)
	{
		...
		new string[128];
    	format(string, sizeof string, "```\n%s [ID: %d] Leave The Server(%s)```", ReturnName(playerid), playerid, reasontext);
...
public OnPlayerSpawn(playerid)
{
	...
	TextDrawShowForPlayer(playerid, ServerTD[5]);
	TextDrawShowForPlayer(playerid, ServerTD[6]);
	TextDrawShowForPlayer(playerid, ServerTD[7]);
	TextDrawShowForPlayer(playerid, ServerTD[8]);
	TextDrawShowForPlayer(playerid, ServerTD[9]);
	TextDrawShowForPlayer(playerid, ServerTD[10]);
    TextDrawShowForPlayer(playerid, ServerTD[11]);
...
public OnPlayerText(playerid, text[])
{
	...
	new msg[128];
    format(msg, sizeof(msg), "```%s: %s```", pData[playerid][pName], text);
	...
	else if(newstate == PLAYER_STATE_DRIVER)
    {
		foreach(new pv : PVehicles)
		{
			if(vehicleid == pvData[pv][cVeh])
			{
				...
				else if(pvData[pv][cDepositor] > 0)
				{
					TextDrawShowForPlayer(playerid, Crate[0]);
					TextDrawShowForPlayer(playerid, Crate[1]);
					PlayerTextDrawShow(playerid, PlayerCrate[playerid]);
					new String2[212];
					format(String2,sizeof(String2),"Money %d packs", pvData[pv][cDepositor]);
					PlayerTextDrawSetString(playerid, PlayerCrate[playerid], String2);
				}
		...
		if(IsATrashVeh(vehicleid))
		{
			ShowPlayerDialog(playerid, DIALOG_TRASHMASTER, DIALOG_STYLE_MSGBOX, "Side Job - TrashMaster", "Anda akan bekerja sebagai trashmaster?", "Start Job", "Close");
		}
		if(IsAPizzaVeh(vehicleid))
		{
...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	if(SedangAnterPizza[playerid] == 1) // pizza
	{
    	...
    	AddPlayerSalary(playerid, "Sidejob(Pizza)", "Pizza courier", 7700);
    	new fmt_str[1280];
    	...
    	SendClientMessage(playerid, COLOR_ARWIN, "PIZZA JOB: {ffffff}Kamu berhasil mengirimkan pizza dan mendapat delay 10 menit.");
        SendClientMessage(playerid, COLOR_ARWIN, fmt_str);
        DisablePlayerCheckpoint(playerid);
	}
...
public OnPlayerEditDynamicObject(playerid, STREAMER_TAG_OBJECT: objectid, response, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz)
{
	if(response == EDIT_RESPONSE_FINAL)
	{
		...
		else if(gymEdit[playerid] == 1)
		{
			new etid =  gymEditID[playerid];
    		GYMInfo[etid][GYMOBJPosX] = x;
			GYMInfo[etid][GYMOBJPosY] = y;
			GYMInfo[etid][GYMOBJPosZ] = z;
			GYMInfo[etid][GYMOBJPosRX] = rx;
			GYMInfo[etid][GYMOBJPosRY] = ry;
			GYMInfo[etid][GYMOBJPosRZ] = rz;
			SetDynamicObjectPos(objectid, GYMInfo[etid][GYMOBJPosX], GYMInfo[etid][GYMOBJPosY], GYMInfo[etid][GYMOBJPosZ]);
	        SetDynamicObjectRot(objectid, GYMInfo[etid][GYMOBJPosRX], GYMInfo[etid][GYMOBJPosRY], GYMInfo[etid][GYMOBJPosRZ]);
			Streamer_SetFloatData(STREAMER_TYPE_3D_TEXT_LABEL, GYMInfo[etid][GYMOBJText], E_STREAMER_X, GYMInfo[etid][GYMOBJPosX]);
			Streamer_SetFloatData(STREAMER_TYPE_3D_TEXT_LABEL, GYMInfo[etid][GYMOBJText], E_STREAMER_Y, GYMInfo[etid][GYMOBJPosY]);
			Streamer_SetFloatData(STREAMER_TYPE_3D_TEXT_LABEL, GYMInfo[etid][GYMOBJText], E_STREAMER_Z, GYMInfo[etid][GYMOBJPosZ] + 0.3);
			SaveGYMObject(etid);
			gymEdit[playerid] = -1;
...
ConvertTimestamp(Time:timestamp, bool:fullDate = true, bool:onlyTime = false, bool:onlyDate = false)
{
    new output[256], tm[e_tm];
    localtime(timestamp, tm);

    if (fullDate) strftime(output, sizeof(output), "%a %d %b %Y, %H:%M:%S", tm);
    else if (onlyTime) strftime(output, sizeof(output), "%H:%M:%S", tm);
    else if (onlyDate) strftime(output, sizeof(output), "%d/%m/%Y", tm);
