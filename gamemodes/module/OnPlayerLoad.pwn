...
SetPlayerSpawn(playerid)
{
	if(IsPlayerConnected(playerid))
	{
		...
		else
		{
			...
			new string[128];
   			format(string, sizeof string, "```\n%s [ID: %d] Connecting To Server```", pData[playerid][pName], playerid);
...
function OnPlayerDataLoaded(playerid, race_check)
{
	...
	if(cache_num_rows() > 0)
	{
		...
		{
			new String[512];
			format(String, sizeof(String), ""YELLOW_E"Your UCP account is blocked\n{00FFFF}User: %s\n{00FFFF}Reason: "WHITE_E"%s\n"YELLOW_E"Please create an unban appeal in our discrod Halena Civilization Roleplay", charData[playerid][cName], BannedReason);
...
function AssignPlayerData(playerid)
{
	...
	cache_get_value_name_int(0, "togmoney", pData[playerid][pTogMoney]);
	...
	cache_get_value_name_int(0, "hasboxing", pData[playerid][pHasBoxing]);
	cache_get_value_name_int(0, "haselbow", pData[playerid][pHasElbow]);
	cache_get_value_name_int(0, "haskneehead", pData[playerid][pHasKneehead]);
	cache_get_value_name_int(0, "haskungfu", pData[playerid][pHasKungfu]);
	cache_get_value_name_int(0, "hasgrabkick", pData[playerid][pHasGrabkick]);
	...
	cache_get_value_name_int(0, "hasflat", pData[playerid][pFlatOwner]);
	cache_get_value_name_int(0, "hashouse1", pData[playerid][pHouseOwner1]);
	cache_get_value_name_int(0, "hashouse2", pData[playerid][pHouseOwner2]);
	cache_get_value_name_int(0, "hasbisnis1", pData[playerid][pBisnisOwner1]);
	cache_get_value_name_int(0, "hasbisnis2", pData[playerid][pBisnisOwner2]);
	cache_get_value_name_int(0, "hasotherprop1", pData[playerid][pOtherPropOwner1]);
	cache_get_value_name_int(0, "hasotherprop2", pData[playerid][pOtherPropOwner2]);
	cache_get_value_name_int(0, "hasotherprop3", pData[playerid][pOtherPropOwner3]);
	cache_get_value_name_int(0, "depositor", pData[playerid][pDepositor]);
