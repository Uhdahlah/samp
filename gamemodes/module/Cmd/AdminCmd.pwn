CMD:acmds(playerid)
{
	...
	strcat(line3, ""WHITE_E"Staff Administrator Commands:"LB2_E"\n\
 	/aduty /a /h /asay /togooc /o /goto /sendto /gethere /getcord /freeze /unfreeze /revive /spec /slap\n\
	...
	/setmaterial /setcomponent /createpv /destroypv /createdealership /explode\n\n\
...
CMD:a(playerid, params[])
{
	...
	new mstr[512];
	/*format(mstr, sizeof(mstr), COLOR_GREEN, " %s(%i): %s ", pData[playerid][pAdminname], playerid, params);*/
	SendAdminMessage(COLOR_GREEN, "%s {FFFF00}%s: {FFFFFF}%s", GetStaffRank(playerid), pData[playerid][pAdminname], params);
	foreach(new ii : Player)
	{
...
CMD:givemoney(playerid, params[])
{
	...
	Servers(playerid, "Kamu telah memberikan uang %s(%d) sebesar $%s!", pData[otherid][pName], otherid, FormatMoney(money));
	Servers(otherid, "Admin %s telah memberikan uang kepada anda sebesar $%s!",pData[playerid][pAdminname], FormatMoney(money));
...
CMD:givebankmoney(playerid, params[])
{
	...
	pData[otherid][pBankMoney] = pData[otherid][pBankMoney]+money;

	Servers(playerid, "Kamu telah memberikan uang rekening bank %s(%d) sebesar $%s!", pData[otherid][pName], otherid, FormatMoney(money));
	Servers(otherid, "Admin %s telah memberikan uang rekening bank kepada anda sebesar $%s!",pData[playerid][pAdminname], FormatMoney(money));
