...
Bisnis_Reset(id)
{
	if(pData[bData[id][bOwnerId]][pBisnisOwner1] == id)
	{
		pData[bData[id][bOwnerId]][pBisnisOwner1] = -1;
	}
	else
	{
		pData[bData[id][bOwnerId]][pBisnisOwner2] = -1;
	}
    ...
    if(id != -1)
    {
		switch(bData[id][bType])
		{
			...
			case 2:
			{
				...
				{
					format(bData[id][bPName2], 128, "Gas Fuel");
				}
				if(strcmp(bData[id][bPName3], "-"))
				{
				}
				...
				{
					format(bData[id][bPName3], 128, "Bandage");
				}
				...
				{
					format(bData[id][bPName4], 128, "Cigarretes");
				}
				...
				{
					format(bData[id][bPName5], 128, "Toll Card");
				}
			}	
			case 3:
			{
...
Bisnis_Type(bisid)
{
	...
	{ //-2240.468505,137.060440,1035.414062	
		...
		bData[bisid][bInt] = 6;
		bData[bisid][bPointX] = -2235.4143;
		bData[bisid][bPointY] = 130.1577;
		bData[bisid][bPointZ] = 1035.4141;
	}
}
...
CMD:createbisnis(playerid, params[])
{
	...
	new type;
	if(sscanf(params, "d", type)) return Usage(playerid, "/createbisnis [type, 1.Fastfood 2.Market 3.Clothes 4.Sportshop 5.Electronic]");
