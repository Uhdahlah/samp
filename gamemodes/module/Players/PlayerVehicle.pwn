...
function LoadPlayerVehicle(playerid)
{
	...
	if(count > 0)
	{
		for(new z = 0; z < count; z++)
		{
			...
			cache_get_value_name_int(z, "depositor", pvData[i][cDepositor]);
...
function RemovePlayerVehicle(playerid)
{
	foreach(new i : PVehicles)
	{
		...
		if(pvData[i][cOwner] == pData[playerid][pID])
		{
			...
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`depositor`=%d, ", cQuery, pvData[i][cDepositor]);
...
CMD:createpv(playerid, params[])
{
	...
	if(pData[otherid][pFlatOwner] == -1)  slotflat = 0;
	else if(pData[otherid][pFlatOwner] > -1) slotflat = 1;
	if(pData[otherid][pHouseOwner1] == -1) slothouse1 = 0;
	else if (pData[otherid][pHouseOwner1] > -1) slothouse1 = 1;
	if(pData[otherid][pHouseOwner2] == -1) slothouse2 = 0;
	else if (pData[otherid][pHouseOwner2] > -1) slothouse2 = 1;
	new count = 0, limit = MAX_PLAYER_VEHICLE + slotflat + slothouse1 + slothouse2 + pData[otherid][pVip];
	foreach(new ii : PVehicles)
	{
...
CMD:sellpv(playerid, params[])
{
	...
	new vehid;
	if(sscanf(params, "d", vehid)) return Usage(playerid, "/sellpv [vehid] | /mv - for find vehid");
	if(vehid == INVALID_VEHICLE_ID) return Error(playerid, "Invalid id");
...
{		
	...
	else Error(playerid, "Vehicle is not ready to be claimed");
	return 1;
}
