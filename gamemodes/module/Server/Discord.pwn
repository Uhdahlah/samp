...
DiscordOnGameMode()
{
	PanelWhitelist = DCC_FindChannelById("1061088650391191663");
	VIP = DCC_FindChannelById("1063953268209885244");
	VIPLogs = DCC_FindChannelById("1063956921792606238");
	role = DCC_FindRoleById("897345217454280718");
	serverid = DCC_FindGuildById("897345217445900308");
	referralcode = DCC_FindChannelById("1063952236071370783");
	referralreddem = DCC_FindChannelById("1063952236071370783");
	activecs = DCC_FindChannelById("1061091464190374019");
	banneducp = DCC_FindChannelById("1061090717260320778");
	bannedchar = DCC_FindChannelById("1061090845266292807");
	
	g_Discord_Chat = DCC_FindChannelById("897345220247683121");
}
...
function OnUserVerif(strr[])
{
	...
	ucp = DCC_CreateEmbed("Halena Project");
	DCC_SetEmbedTitle(ucp, "HALENA CIVILIZATION ROLEPLAY");
	DCC_SetEmbedTimestamp(ucp, timestamp);
	DCC_SetEmbedColor(ucp, 0xff0000);
	DCC_SetEmbedUrl(ucp, "");
	DCC_SetEmbedFooter(ucp, "HALENA CIVILIZATION", "");
	DCC_SetEmbedDescription(ucp, strr);
	DCC_SetEmbedImage(ucp, "https://cdn.discordapp.com/icons/897345217445900308/ca250afd3f817682a7c2af88db3a6ece.png");
...
DCMD:ucp(user, channel, params[])
{
    if(channel != PanelWhitelist)
		return DCC_SendChannelMessage(channel, "Anda tidak bisa melakukan perintah tersebut di channel ini");
    ...
    if(cache_num_rows() == 1) return DCC_SendChannelMessage(PanelWhitelist, "**UCP: :x: Nama UCP tersebut sudah dipakai!**");
	...
	format(str, 250, "**REGISTER UCP**\nSelamat Karakter anda di Halena Civilization Roleplay berhasil didaftarkan Gunakan UCP untuk login di dalam server!\n```\nUCP: %s\nPIN: %d```\n**IP SERVER: %s\n**", params, hu, IP_SERVER);
	new DCC_Embed:acc, stri[2800];
	acc = DCC_CreateEmbed("HCRP");
	format(stri, 280, "**Register UCP**\n***%s*** successfully verified, please check the message", params);
	new y, m, d, timestamp[200];
	getdate(y, m , d);
	format(timestamp, sizeof(timestamp), "%02i%02i%02i", y, m, d);
	DCC_SetEmbedTitle(acc, "Halena Project");
	DCC_SetEmbedTimestamp(acc, timestamp);
	DCC_SetEmbedColor(acc, 0x00ff00);
	DCC_SetEmbedThumbnail(acc, "https://cdn.discordapp.com/icons/897345217445900308/ca250afd3f817682a7c2af88db3a6ece.png");
	DCC_SetEmbedFooter(acc, "Halena", "");
	...
	mysql_format(g_SQL, tquery, sizeof tquery, "INSERT INTO `ucp` (`username`, `pin`) VALUES ('%e', '%d', '%d')", params, hu);
    mysql_tquery(g_SQL, tquery);
	
	return 1;
}

DCMD:banucp(user, channel, params[])
{
...
DCMD:unbanucp(user, channel, params[])
{
    if(channel != banneducp)
		return DCC_SendChannelMessage(channel, "Anda tidak bisa melakukan perintah tersebut di channel ini");
...
DCMD:banchar(user, channel, params[])
{
...
DCMD:unbanchar(user, channel, params[])
{
    if(channel != bannedchar)
		return DCC_SendChannelMessage(channel, "Anda tidak bisa melakukan perintah tersebut di channel ini");
...
{
    new statuz[256];
    format(statuz,sizeof(statuz),"Halena Civilization Roleplay | #HCRP",playeronline);
