...
CMD:loadmap(playerid, params[]) {
  ...
  if (sscanf(params, "s[64]ddD(-1)D(-1)", mapname, world, interior, houseid, bizid))
    return Usage(playerid, "/loadmap [map name] [world] [interior] [opt: house id] [opt: biz id]");

  new id = cellmin;
  if ((id = Iter_Free(Maps)) != cellmin) {
    if (bizid != -1 && !Iter_Contains(Bisnis, bizid))
      return Error(playerid, "Invalid biz id!");
    ...
    mysql_tquery(g_SQL, "INSERT INTO `maps` (`Name`) VALUES ('%s')", mapname, "OnMapCreated", "d", id);
  } else Error(playerid, "Server has reached the maximum of maps (MAX: %d).", MAX_MAP);
  return 1;
}

function OnMapCreated(id) {
  MapData[id][eID] = cache_insert_id();
  mysql_tquery(g_SQL, "UPDATE `maps` SET `Name`='%s' WHERE `ID`='%d'",MapData[id][eName],MapData[id][eID]);
  return 1;
}
...
Map_Import(playerid, name[], world, interior, houseid = -1, bizid = -1) {
  ...
  format(dir,sizeof(dir),"maps/%s.txt",name);
  if(fexist(dir))
  {
    ...
    while(fread(hFile,string,sizeof(string)))
    {
      ...
      if (type == 1) {
        if(!sscanf(string,"p<,>dffffff",model,pos[0],pos[1],pos[2],rot[0],rot[1],rot[2])) {
          ...
          if (houseid != -1) ObjData[slot][oHouse] = hData[houseid][hID];
          if (bizid != -1) ObjData[slot][oBiz] = bData[bizid][BiznisID];
          loaded++;
        }
      }
      if (type == 3) {
    ...
    fclose(hFile);
    SendClientMessageEx(playerid, COLOR_ARWIN, "OBJECT "YELLOW_E"%d objects "WHITE_E"loaded from file '"GREEN_E"%s"WHITE_E"'",loaded,dir);
  } else Error(playerid, "The file doesn't exists!");
  return 1;
}
