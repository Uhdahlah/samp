...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == DIALOG_GENDER)
    {
		...
		if(response)
		{
			...
			SendClientMessageEx(playerid, COLOR_ARWIN, "ACCOUNT: "WHITE_E"You've arrive at Unity Station with "GREEN_E"$100.00 {FFFFFF}in your hand and "GREEN_E"$100.00 {FFFFFF}on your Bank Account");
		}
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					for(new i = 0; i < 33; i++)
					{
						PlayerTextDrawHide(playerid, BARTD[playerid][i]);
					}
					pData[playerid][pHBEMode] = 1;
			...
			else if(bData[bizid][bType] == 2)
			{
				switch(listitem)
				{
					...
					case 5:
					{
						...
						SendClientMessageEx(playerid, COLOR_ARWIN, "SHOP: {ffffff}You've purchased {ffff00}%s {ffffff}for {00ff00}$%s", bData[bizid][bPName5], FormatMoney(price));
						bData[bizid][bProd]--;
						bData[bizid][bMoney] += Server_Percent(price);
						Server_AddPercent(price);
						Bisnis_Save(bizid);
	...
	if(dialogid == DIALOG_SELL_HOUSE)
	{
		if(response)
		{
			...
			if(pData[playerid][pHouseOwner1] == hid)
			{
				pData[playerid][pHouseOwner1] = -1;
			}
			else
			{
				pData[playerid][pHouseOwner2] = -1;
			}
			GivePlayerMoneyEx(playerid, 2);
	...
	if(dialogid == DIALOG_LOCKVEHICLE)
	{
		if(response) 
		{
			...
			if(pvData[carid][cOwner] == pData[playerid][pID] && pvData[carid][cImpound] == 0 && pvData[carid][cClaim] == 0)
			{
				...
				{
					SendClientMessageEx(playerid, COLOR_GREY, "You have to be near your car.");
				}
			}
		}
		return 1;
	...
	if(dialogid == DIALOG_HELP)
    {
		...
		switch(listitem)
		{
			case 0:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Players Help", "/drag /undrag /items /frisk /give /pay /stats /use /togmask /weapon /deny /accept /accent /eject /showlicense /showvehlic /buy /helmet /death /settings /togauto", "Ok", "Kembali");
			}
			case 1:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Chat", "/n /l /s /pm /togpm /w /o /me /ame /ado /do /try", "Ok", "Kembali");
			}
			case 2:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_LIST, "Money & Banking", "/pay /mysalary /atm /balance /withdraw /deposit /transfer /signcheck", "Select", "Kembali");
			}
			case 3:
			{
				ShowPlayerDialog(playerid, DIALOG_HELP_JOBS, DIALOG_STYLE_LIST, "Job Help", "{00FFFF}Job: "WHITE_E"Taxi Drivers\n{00FFFF}Job: "WHITE_E"Mechanic\n{00FFFF}Job: "WHITE_E"Lumber Jack\n{00FFFF}Job: "WHITE_E"Trucker\n{00FFFF}Job: "WHITE_E"Farmer\n{00FFFF}Job: "WHITE_E"Builder\n{00FFFF}Job: "WHITE_E"Depositor\n"YELLOW_E"Sidejob: "WHITE_E"TrashMaster\n"YELLOW_E"Sidejob: "WHITE_E"Pizza\n"YELLOW_E"Sidejob: "WHITE_E"Forklift\n"RED_E"IlegalJob: "WHITE_E"Arms Dealer\n"RED_E"IlegalJob: "WHITE_E"Drugs Dealer\n"RED_E"IlegalJob: "WHITE_E"Smuggle", "Select", "Close");
				return 1;
			}
			case 4:
			{
			...
			case 9:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Business Commands", "/buy /bm /lockbisnis /unlockbisnis /bwithdraw /bdeposit", "Ok", "Kembali");
			}
			case 10:
			{
	...
	if(dialogid == DIALOG_HELP_JOBS)
    {
		...
		switch(listitem)
		{
			...
			case 6:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Depositor Help", "/loadbankmoney /takebankmoney /atmdeposit", "Ok", "Kembali");
			}
			case 7:
			{
			...
			case 8:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Pizza Help", "/getpizza", "Ok", "Kembali");
			}
			case 9:
			{
	...
	if(dialogid == DIALOG_GPS_FURN)
	{
		if(response)
		{
			new furnstoreid = gListedItems[playerid][listitem];

			SetPlayerRaceCheckpoint(playerid, 1, storeData[furnstoreid][storePos][0], storeData[furnstoreid][storePos][1], storeData[furnstoreid][storePos][2], 0,0,0, 5);
			SendClientMessageEx(playerid, COLOR_ARWIN, "GPS: "WHITE_E"Furniture Store Checkpoint targeted! (%s)", GetLocation(storeData[furnstoreid][storePos][0], storeData[furnstoreid][storePos][1], storeData[furnstoreid][storePos][2]));
		}
	}
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 7:
				{
					...
					{
						if(strcmp(CarDealershipInfo[id][cdOwner], "-"))
						{
							gListedItems[playerid][found] = id;
				...
				case 8:
				{
					new msg2[512], found = false;
					format(msg2, sizeof(msg2), "Name\tDistance\n");
					for(new id = 0; id < MAX_FURNSTORE; id++)
					{
						if(strcmp(storeData[id][storeOwner], "-"))
						{
							gListedItems[playerid][found] = id;
							format(msg2,sizeof(msg2), "%s%s\t%0.2fm\n", msg2, storeData[id][storeName], GetPlayerDistanceFromPoint(playerid,storeData[id][storePos][0], storeData[id][storePos][1], storeData[id][storePos][2]));
							found++;
						}
					}
					ShowPlayerDialog(playerid, DIALOG_GPS_FURN, DIALOG_STYLE_TABLIST_HEADERS,"GPS: Furniture Store",msg2,"Goto","Cancel");
				}
				case 9:
				{
					new msg2[512], found = false;
	...
	if(dialogid == DIALOG_GPS_SIDEJOB)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 4:
				{
					SetPlayerRaceCheckpoint(playerid, 1, 2108.7407,-1785.5049,13.3868, 0.0, 0.0, 0.0, 3.5);
					SendClientMessageEx(playerid, COLOR_ARWIN, "HINT: "WHITE_E"GPS active! follow the checkpoint.");
				}
				case 5:
				{
	...
	if(dialogid == DIALOG_GPS_JOB)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 4:
				{
					SetPlayerRaceCheckpoint(playerid,1, 1613.6221,-1892.9106,13.5469, 0.0, 0.0, 0.0, 3.5); //Depositor
					SendClientMessageEx(playerid, COLOR_ARWIN, "HINT: "WHITE_E"GPS active! follow the checkpoint.");
				}
				case 5:
				{
	...
	if(dialogid == DIALOG_GMENU)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 2:
				{
					ShowPlayerDialog(playerid, DIALOG_BFSTYLE, DIALOG_STYLE_LIST, "Fighting Moves", "Boxing\t$200.00\nElbow\t$300.00\nKneehead\t$400.00\nKung-Fu\t$400.00\nGrabKick	$200.00", "Learn", "Cancel");
				}
			}
		}
	}
	if(dialogid == DIALOG_BFSTYLE)
	{
		if(response)
		{
			if(GetPlayerMoney(playerid) >= 20000)
			{
				if(listitem == 0)
				{
					pData[playerid][pHasBoxing] = 1;
			    	pData[playerid][FightStyle] = FIGHT_STYLE_BOXING;
				...
				if(listitem == 1)
				{
					pData[playerid][pHasElbow] = 1;
					pData[playerid][FightStyle] = FIGHT_STYLE_ELBOW;
				...
				if(listitem == 2)
				{
					pData[playerid][pHasKneehead] = 1;
			    	pData[playerid][FightStyle] = FIGHT_STYLE_KNEEHEAD;
				...
				if(listitem == 3)
				{
					pData[playerid][pHasKungfu] = 1;
   					pData[playerid][FightStyle] = FIGHT_STYLE_KUNGFU;
				...
				if(listitem == 4)
				{
					pData[playerid][pHasGrabkick] = 1;
					pData[playerid][FightStyle] = FIGHT_STYLE_GRABKICK;
	...
	if(dialogid == DIALOG_FIGHTSTYLE)
	{
		if(response)
        {
			...
			if(id == 1)
			{
				pData[playerid][FightStyle] = FIGHT_STYLE_NORMAL;
	...
	if(dialogid == DIALOG_PIZZA)
	{
		...
		if(response)
		{
			...
			{
				Error(playerid, "Anda harus menunggu "GREY2_E"%d "WHITE_E"detik lagi.", pData[playerid][pPizzaTime]);
				RemovePlayerFromVehicle(playerid);
				SetTimerEx("RespawnPV", 3000, false, "d", vehicleid);
				return 1;
			}
			pData[playerid][pSideJob] = 501;
			SendClientMessage(playerid, COLOR_ARWIN, "PIZZA SIDEJOB: {ffffff}Ikuti checkpoint pada map dan hati hati dijalan!.");
    ...
    if(dialogid == DIALOG_TRASHMASTER)
	{
		...
		if(response)
		{
			...
			{
				Error(playerid, "Anda harus menunggu "GREY2_E"%d "WHITE_E"detik lagi.", pData[playerid][pSideJobsTrash]);
				RemovePlayerFromVehicle(playerid);
				SetTimerEx("RespawnPV", 3000, false, "d", vehicleid);
				return 1;
			}
			pData[playerid][pSideJob] = 301;
			SetPlayerMapIcon(playerid, 20, trashpoint1, 0, 3, 0);
			SetPlayerMapIcon(playerid, 21, trashpoint2, 0, 3, 0);
			SetPlayerMapIcon(playerid, 22, trashpoint3, 0, 3, 0);
			SetPlayerMapIcon(playerid, 23, trashpoint4, 0, 3, 0);
			SetPlayerMapIcon(playerid, 24, trashpoint5, 0, 3, 0);
			SetPlayerMapIcon(playerid, 25, trashpoint6, 0, 3, 0);
			SetPlayerMapIcon(playerid, 26, trashpoint7, 0, 3, 0);
			SetPlayerMapIcon(playerid, 27, trashpoint8, 0, 3, 0);
			SetPlayerMapIcon(playerid, 28, trashpoint9, 0, 3, 0);
			InfoTD_MSG(playerid, 3000, "Follow the checkpoint!");
		}
		else
		{
