...
CMD:finvite(playerid, params[])
{
	...
	if(pData[otherid][pCS] < 0)
		return Error(playerid, "This player Character Story has not been aproved!");
    if(sscanf(params, "u", otherid))
        return Usage(playerid, "/finvite [playerid/PartOfName]");
...
CMD:f(playerid, params[])
{
	...
	SendFamilyMessage(pData[playerid][pFamily], COLOR_FAMILY, "(( "FAMILY_E"[F][%s] %s: %s ))", GetFamilyRank(playerid), pData[playerid][pName], params);
    return 1;
}
