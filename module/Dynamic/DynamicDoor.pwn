...
Doors_Updatelabel(id)
{
	if(id != -1)
	{
		...
		if(dData[id][dGarage] == 1)
		{
			...
			dData[id][dPickupext] = CreateDynamicCP(dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ], 2.0, -1, -1, -1, 5.0);
		...
		{
			new mstr[512];
			format(mstr,sizeof(mstr),"{00FFFF}[ID: %d]\n{FFFF00}%s\n{FFFFFF}Press '{FF0000}ENTER{FFFFFF}' to enter/exit the door", id, dData[id][dName]);
			dData[id][dPickupext] = CreateDynamicPickup(19130, 23, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ], dData[id][dExtvw], dData[id][dExtint]);
            dData[id][dPickupext] = CreateDynamicCP(dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ], 2,  0, -1, -1, 5.0);
			dData[id][dLabelext] = CreateDynamic3DTextLabel(mstr, COLOR_YELLOW, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ]+0.35, 15.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, dData[id][dExtvw], dData[id][dExtint]);
		}
        ...
        if(dData[id][dIntposX] != 0.0 && dData[id][dIntposY] != 0.0 && dData[id][dIntposZ] != 0.0)
        {
			if(dData[id][dGarage] == 1)
			{
                ...
                dData[id][dPickupint] = CreateDynamicCP(dData[id][dIntposX], dData[id][dIntposY], dData[id][dIntposZ], 2.0, -1, -1, -1, 5.0);
			}
			else
			{
				new mstr[512];
...
CMD:createdoor(playerid, params[])
{
	...
	new String[212];
	format(String, sizeof(String), "AdmWarn: "YELLOW_E"%s "WHITE_E"telah membuat Door ID "YELLOW_E"%d.", pData[playerid][pAdminname], did);
...
CMD:editdoor(playerid, params[])
{
    ...
    else if(!strcmp(type, "virtual", true))
    {
        ...
        Doors_Save(did);
		Doors_Updatelabel(did);
        SendAdminMessage(COLOR_RED, "%s has adjusted the virtual of entrance ID: %d", pData[playerid][pAdminname], did);
    }
